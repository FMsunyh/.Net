...
namespace Fly
{
    ...
    public class EnemyEighteen : Roles
    {
        ...
        protected override void ThisBomb()
        {
            ...
            //爆炸的时候放出 血块+西瓜
            if (MainForm.m_EnemyRandom.Next(0, 100) < 100)
            {
                HitCheck.GetInstance().AddElement(new EatBlood(this, this.Good, 5, 5, MissileDirection.STOP, -30));
                HitCheck.GetInstance().AddElement(new EatMissile(this, this.Good, 3, 8, MissileDirection.STOP, 0));

                HitCheck.GetInstance().AddElement(new MedalFive(this, 0, MainForm.m_RollSpeed, 5));
            }
